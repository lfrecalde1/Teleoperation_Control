...
th1_s = state_s(2);
% dimensions
W_s = 1*sqrt(m_0_s/5);  % cart width
H_s = 1*sqrt(m_0_s/5); % cart height
mr_s =0.2*sqrt(m_1_s);  % mass radius
mr1_s=0.2*sqrt(m_2_s);

% Master parameters
th_m = state_m(1);
th1_m = state_m(2);
% dimensions
W_m = 1*sqrt(m_0_m/5);  % cart width
H_m = 1*sqrt(m_0_m/5); % cart height
mr_m =0.2*sqrt(m_1_m);  % mass radius
mr1_m=0.2*sqrt(m_2_m);
